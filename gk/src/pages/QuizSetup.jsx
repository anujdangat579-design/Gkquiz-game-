...
import { watchCompetitionConfig, getFeeForDifficulty } from '../lib/config'

export default function QuizSetup({ user }) {
  const navigate = useNavigate()
  const { state } = useLocation()
  const presetCategory = state?.presetCategory || null
  ...
  useEffect(() => {
    const unsub = watchCompetitionConfig((c) => {
      setConfig(c)
      // A subject tapped on Home wins over the default — but only if it's
      // actually a category admin has enabled, so a stale/removed subject
      // can't leave the picker stuck on something invalid.
      setCategory((cur) => cur ?? (presetCategory && c.enabledCategories.includes(presetCategory) ? presetCategory : c.enabledCategories[0]))
  ...
  return (
    <div className="screen">
      ...
      <h1 className="display" style={{ fontSize: 24, margin: '4px 0 20px' }}>
        {presetCategory ? `${presetCategory} Quiz` : 'Choose your quiz'}
      </h1>
      ...
      <div className="grid-3" style={{ marginBottom: 22 }}>
        {config.enabledDifficulties.map((d) => (
          <button
            ...
            style={{ justifyContent: 'center', flexDirection: 'column', gap: 2, lineHeight: 1.3 }}
            onClick={() => setDifficulty(d)}
          >
            <span>{d}</span>
            <span style={{ fontSize: 11, opacity: 0.8 }}>₹{getFeeForDifficulty(config, d)}</span>
          </button>
        ))}
      </div>
      ...
      <div className="card" style={{ marginBottom: 20, background: 'var(--surface-2)' }}>
        <p className="eyebrow" style={{ color: 'var(--violet)' }}>ACCESS FEE · {difficulty}</p>
        <p className="display" style={{ fontSize: 28, margin: '4px 0' }}>₹{getFeeForDifficulty(config, difficulty)}</p>
        <p className="muted" style={{ fontSize: 12, lineHeight: 1.5 }}>
          Covers quiz competition access and your educational score report. Fee scales with the
          difficulty you pick above. No cash prize is paid to the winner and no money can be
      ...
      <button className="btn-primary" onClick={startMatchmaking} disabled={paying}>
        {paying ? 'Processing payment…' : `Pay ₹${getFeeForDifficulty(config, difficulty)} & Find Opponent`}
      </button>
      <p className="muted" style={{ fontSize: 11, textAlign: 'center', marginTop: 10 }}>
        Secure payment via Cashfree. No cash prize is paid to the winner and no money can be
        withdrawn on this platform.
      </p>
    </div>
  )

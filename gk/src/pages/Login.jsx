...
export default function Login({ isGuestUpgrade, onAuthResult }) {
  ...
  function handleDisclaimerTap() {
    const now = Date.now()
    const recent = (window.__gkAdminTaps || []).filter((t) => now - t < HIDDEN_TAP_WINDOW_MS)
    recent.push(now)
    window.__gkAdminTaps = recent
    if (recent.length >= HIDDEN_TAP_TARGET) {
      window.__gkAdminTaps = []
  ...
  return (
    <div className="screen" style={{ justifyContent: 'center' }}>
      ...
      <p className="muted" style={{ fontSize: 11, textAlign: 'center', marginTop: 22, lineHeight: 1.6 }}>
        <span onClick={handleDisclaimerTap}>By continuing you agree to our{' '}</span>
      ...
      {showAdminAccess && (
        <AdminAccessForm
          onAuthResult={(u) => { onAuthResult(u); navigate('/admin') }}
...
// Hidden admin entry point — revealed by tapping the disclaimer text 5x
// within 3 seconds (see handleDisclaimerTap above). Deliberately reuses the
// same real Firebase email/password sign-in as the regular Email tab rather
// than a hardcoded password: a secret baked into client-side JS can always
// be read via "View Source" / DevTools, so the actual gate has to stay the
// Firebase custom admin claim checked in AdminGate.jsx, not this form.
function AdminAccessForm({ onAuthResult, onClose }) {
  ...
  return (
    <div className="card" style={{ marginTop: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        <p style={{ fontWeight: 700, fontSize: 13 }}>Admin Sign In</p>
        <button className="muted" style={{ fontSize: 12, background: 'none', border: 'none' }} onClick={onClose}>✕</button>
      </div>
      {error && <p style={errStyle}>{error}</p>}
      <input style={inputStyle} type="email" placeholder="Admin email" value={email} onChange={(e) => setEmail(e.target.value)} />
      <input style={{ ...inputStyle, marginTop: 8 }} type="password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} />
      <button className="btn-primary" style={{ marginTop: 10 }} onClick={submit} disabled={busy || !email || !password}>

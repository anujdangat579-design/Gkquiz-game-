...
// ---------------------------------------------------------------------------
// Winner Messages — post-match note from the winner to the admin, plus the
// admin's reply. Direct client writes (no Cloud Function) since no money
// moves here; firestore.rules restricts a reply write to admin-only and to
// exactly the {adminReply, status, repliedAt} fields.
// ---------------------------------------------------------------------------
export function watchWinnerMessagesAdmin(callback) {
  const q = query(collection(db, 'winnerMessages'), orderBy('createdAt', 'desc'), limit(200))
  return onSnapshot(q, (snap) => callback(snap.docs.map((d) => ({ id: d.id, ...d.data() }))))
}

export async function replyToWinnerMessage(id, replyText) {
  const text = (replyText || '').trim()
  if (!text) throw new Error('Write a reply before sending.')
  await updateDoc(doc(db, 'winnerMessages', id), {
    adminReply: text,
    status: 'replied',

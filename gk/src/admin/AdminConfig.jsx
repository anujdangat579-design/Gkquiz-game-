...
export default function AdminConfig() {
  ...
  async function save() {
    setError(null)
    setMessage(null)
    const fee = Number(form.entryFeeRupees)
  ...
  return (
    <div>
      <div className="card" style={{ marginBottom: 16 }}>
        <p style={{ fontWeight: 700, marginBottom: 6 }}>Entry fee</p>
        <p className="muted" style={{ fontSize: 12, marginBottom: 10, lineHeight: 1.5 }}>
          Charged server-side by the <code>createOrder</code> Cloud Function — changing this here
          takes effect on the very next payment, no redeploy needed.
        </p>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <input
            ...
            onChange={(e) => setForm((f) => ({ ...f, entryFeeRupees: e.target.value }))}
            style={inputStyle}
          />
        </div>
      </div>
...
function toFormState(c) {
  return {
    entryFeeRupees: c.entryFeeRupees ?? DEFAULT_CONFIG.entryFeeRupees,

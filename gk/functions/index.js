...
// Default access fees by difficulty, used until an admin saves a
// config/competition doc (see AdminConfig.jsx) and as a safe fallback if
// that doc is ever missing/malformed. The fee actually charged always
// comes from this function reading Firestore server-side (Admin SDK) — a
// tampered client can never request a different amount or difficulty/fee
// pairing, since it never gets to say what the amount is in the first place.
const DEFAULT_FEE_PAISE_BY_DIFFICULTY = { Easy: 1000, Medium: 2000, Hard: 3000 } // ₹10 / ₹20 / ₹30
...
async function getLiveCompetitionConfig() {
  try {
    ...
    if (!snap.exists) {
      return { feePaiseByDifficulty: DEFAULT_FEE_PAISE_BY_DIFFICULTY, questionCountOptions: DEFAULT_QUESTION_COUNT_OPTIONS }
    }
    const data = snap.data() || {}

    let feePaiseByDifficulty = DEFAULT_FEE_PAISE_BY_DIFFICULTY
    if (data.feeByDifficulty && typeof data.feeByDifficulty === 'object') {
      const built = {}
      for (const [difficulty, rupees] of Object.entries(data.feeByDifficulty)) {
        const n = Number(rupees)
        if (Number.isFinite(n) && n > 0) built[difficulty] = Math.round(n * 100)
      }
      if (Object.keys(built).length > 0) feePaiseByDifficulty = built
    } else if (Number.isFinite(Number(data.entryFeeRupees)) && Number(data.entryFeeRupees) > 0) {
      // Legacy flat-fee configs saved before per-difficulty pricing existed.
      const flatPaise = Math.round(Number(data.entryFeeRupees) * 100)
      feePaiseByDifficulty = { Easy: flatPaise, Medium: flatPaise, Hard: flatPaise }
    }
    ...
    return { feePaiseByDifficulty, questionCountOptions }
  } catch {
    return { feePaiseByDifficulty: DEFAULT_FEE_PAISE_BY_DIFFICULTY, questionCountOptions: DEFAULT_QUESTION_COUNT_OPTIONS }
  }
}
...
// Switch environments with (defaults to sandbox if unset):
...
// Thin wrapper around Cashfree's Orders/Payments/Refunds REST API using the
// runtime's built-in fetch (Node 20) — no extra SDK dependency needed.
async function cashfreeRequest(path, { method = 'GET', body, appId, secretKey } = {}) {
  ...
  if (!res.ok) {
    throw new Error(data?.message || `Cashfree request failed (${res.status})`)
  }
  return data
}
...
/**
 * Creates a Cashfree order for the ₹10 quiz access fee and records a
 * 'created' transaction doc. Called from the client before opening the
 * Cashfree Checkout widget with the returned payment_session_id.
 */
exports.createOrder = onCall(
  ...
  async (request) => {
    ...
    const { feePaiseByDifficulty, questionCountOptions } = await getLiveCompetitionConfig()
    ...
    const feePaise = feePaiseByDifficulty[difficulty]
    if (!Number.isFinite(feePaise) || feePaise <= 0) {
      throw new HttpsError('invalid-argument', 'That difficulty isn\'t available right now.')
    }

    // Cashfree order_id must be unique and alphanumeric (with _ / -).
    const orderId = `order_${uid.slice(0, 12)}_${Date.now()}`
    const phone = request.auth.token?.phone_number || '9999999999'
    const email = request.auth.token?.email || `${uid}@gkquizgame.app`
    ...
    const order = await cashfreeRequest('/orders', {
      ...
      body: {
        ...
        order_note: 'Quiz Competition Access & Educational Assessment Fee',
      },
    })
...
/**
 * Verifies payment server-side (never trust the client's word that a
 * payment succeeded) by asking Cashfree directly for that order's payments,
 * and flips the transaction to 'paid' only if one is SUCCESS. Firestore
 * rules check this doc before allowing a queue entry.
 */
exports.verifyPayment = onCall(
  ...
  async (request) => {
    ...
    if (tx.status === 'paid') return { verified: true, transactionId: orderId } // idempotent retry

    const payments = await cashfreeRequest(`/orders/${orderId}/payments`, {
      appId: CASHFREE_APP_ID.value(),
      secretKey: CASHFREE_SECRET_KEY.value(),
    })
    ...
    if (!successPayment) {
      throw new HttpsError('permission-denied', 'Payment not confirmed yet.')
    }
    ...
    return { verified: true, transactionId: orderId }
  }
)

/**
 * Cashfree webhook fallback — catches payments that succeeded but whose
 ...
 * event: PAYMENT_SUCCESS_WEBHOOK, and set CASHFREE_WEBHOOK_SECRET to match
 * (defaults to your client secret unless you set a dedicated one there).
 */
const { onRequest } = require('firebase-functions/v2/https')
const CASHFREE_WEBHOOK_SECRET = defineSecret('CASHFREE_WEBHOOK_SECRET')

exports.cashfreeWebhook = onRequest(
  { secrets: [CASHFREE_WEBHOOK_SECRET] },
  async (req, res) => {
    const signature = req.headers['x-webhook-signature']
    const timestamp = req.headers['x-webhook-timestamp']
    const body = req.rawBody

    // Cashfree signs timestamp + raw body (not parsed JSON) with HMAC-SHA256, base64-encoded.
...
// through Cashfree. Every step is written to `auditLogs` so there's a
...
/**
 * Admin approves a pending refund request: calls Cashfree's Refunds API,
...
exports.adminApproveRefund = onCall(
  ...
  async (request) => {
    ...
    const cashfreeRefundId = `refund_${refundReq.transactionId}_${Date.now()}`
    const refund = await cashfreeRequest(`/orders/${refundReq.transactionId}/refunds`, {
      method: 'POST',
      appId: CASHFREE_APP_ID.value(),
      secretKey: CASHFREE_SECRET_KEY.value(),
      body: {
        ...
        refund_note: 'Quiz access fee refund approved by admin',
      },
    })
    ...
    await logAudit('refund_approved', adminUid, {
      ...
      cashfreeRefundId: refund.refund_id,
    })

    return { refunded: true, refundId: refund.refund_id }
  }
)

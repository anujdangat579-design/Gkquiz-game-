// Mirrors src/data/questionBank.js (kept in sync manually — this is the
// CommonJS copy Cloud Functions can require directly). Used only by the
// one-time `bulkSeedQuestionBank` admin callable to populate the live
// Firestore `questions` collection so matchmaking has real data to assign
// from instead of relying on the old bundled file.
...
const QUESTION_BANK = [
  ...
  q('Banking GK', 'Easy', 'What does "RBI" stand for?', ['Reserve Bank of India', 'Regional Bank of India', 'Rural Bank of India', 'Registered Bank of India'], 0, "RBI is India's central banking institution, established in 1935."),
  q('Banking GK', 'Medium', 'What is the full form of "NEFT"?', ['National Electronic Funds Transfer', 'National Express Funds Transfer', 'National Electronic Finance Transfer', 'National Exchange Funds Transfer'], 0, 'NEFT allows one-to-one electronic fund transfers between bank accounts.'),
  q('SSC GK', 'Medium', 'Who is known as the "Missile Man of India"?', ['Homi Bhabha', 'A.P.J. Abdul Kalam', 'Vikram Sarabhai', 'C.V. Raman'], 1, 'Dr. A.P.J. Abdul Kalam led key missile and space programs and later became President of India.'),
  q('UPSC GK', 'Hard', 'The Directive Principles of State Policy are inspired by the constitution of which country?', ['USA', 'UK', 'Ireland', 'Canada'], 2, "India's Directive Principles were inspired by the Irish Constitution."),
  q('MPSC GK', 'Medium', 'Which is the largest district in Maharashtra by area?', ['Pune', 'Ahmednagar', 'Nashik', 'Nagpur'], 1, 'Ahmednagar is the largest district in Maharashtra by geographical area.'),
